...
The test suite only runs within a Rails app:

  cd vendor/plugins/translator
  rake

Since they load the environment, you may want to create a fresh Rails app just for running the tests during development:

  rails translator_app
  cd translator_app
  script/plugin install git://github.com/graysky/translator.git
  cd vendor/plugins/translator
  rake
...
1.1.0 - 1/19/2010 - Support namespaced controllers and their views.
